...
// "Opiniones" muestra un estado de espera — no se inventa ningún
// testimonio mientras no haya una fuente real conectada. Para activarla:
//
...
//   3. Añade GOOGLE_PLACE_ID si conoces el Place ID exacto del
//      restaurante (evita una búsqueda extra).
// -----------------------------------------------------------------------
...
async function resolvePlaceId(apiKey: string): Promise<string | null> {
  ...
  const query = "Azzurro restaurante italiano";
  const url =
